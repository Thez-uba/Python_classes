...
# Customer Class
class Customer(User):
    def __init__(self, firstname, lastname, email, phone, gender, nin, home_address, occupation, state_of_origin, lga, dob):
    ...
    def registerCustomer(self, account_number):
        new_customer = {
            "firstname" : self.name,
    ...
    def createAccountForCustomer(self, staff_id, password):
        for staff_user in staff:
            if staff_user["id"] == staff_id and staff_user["password"] == password:
                ...
                email = input("Enter coustomer email: ")
                ...
                state = input("Enter custoer state odf origin: ")
                ...
                customer.resgisterCustomer(account_number, firstname, lastname, email, phone, nin, home, gender, dob, state, lga, occupation)
            elif staff_user["id"] != staff_id and staff_user["password"] != password and staff [-1] == staff_user:
                print("Incorrect login credentisls")
    ...
    def transfer(self, staff_id, password):
        for staff_user in staff:
            if staff_user["id"] == staff_id and staff_user["password"] == password:
                ...
                customer = Customer()
                customer.transferMoney(sender_account, receiver_account)
            elif staff_user["id"] != staff_id and staff_user["password"] != password and staff [-1] == staff_user:
...
class Manager:
    ...
    def restrict_customer_access(username, password):
        if username == manager["username"] and password == manager["password"]:
            customer = Customer()
    ...
    def delete_customer_account(username, password):
        if username == manager["username"] and password == manager["password"]:
            customer = Customer()
    ...
    def view_customers(username, password):
        if username == manager["username"] and password == manager["password"]:
            customer = Customer()
            account_number = input("Enter the customer account to be deleted: ")
...
class MainApp:
    ...
    def appMenu(self):
        ...
        print("Whatdo you want to perform?")
    ...
    def startApp(self):
        ...
        while True:
            choice = input("Enter a number between 1 and 11: ")
            match choice:
                case "1":
